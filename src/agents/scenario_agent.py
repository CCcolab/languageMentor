...
class SalaryNegotiationAgent(ScenarioAgent):
    def chat_with_history(self, user_input):
        # 实现薪资谈判场景的对话逻辑
        if "薪水" in user_input:
            response = "在薪资谈判中，您可以询问：'我希望薪水能提高到多少？'"
        else:
            response = f"在薪资谈判中，你可以说: '{user_input}'"
        return response
...
class RentingAgent(ScenarioAgent):
    def chat_with_history(self, user_input):
        # 实现租房场景的对话逻辑
        if "租金" in user_input:
            response = "在租房过程中，您可以询问：'这个房子的租金是多少？'"
        else:
            response = f"在租房过程中，你可以询问: '{user_input}'"
        return response
...
class LeaveRequestAgent(ScenarioAgent):
    def chat_with_history(self, user_input):
        # 实现单位请假场景的对话逻辑
        if "请假" in user_input:
            response = "请假时，你可以说: '我需要请假几天，原因是...'"
        else:
            response = f"请假时，你可以说: '{user_input}'"
        return response
