...
def handle_conversation(user_input, chat_history):
    ...
    # 修改为调用新的 chat_with_history 方法
    bot_message, examples = conversation_agent.chat_with_history(user_input, chat_history)
    formatted_message = format_chatbot_response(bot_message, examples)
    LOG.info(f"[ChatBot]: {formatted_message}")
    return formatted_message

# 新增格式化 ChatBot 回复的函数
def format_chatbot_response(bot_message, examples):
    formatted_response = (
        f"### 教学点评:\n{bot_message}\n\n"
...
with gr.Blocks(title="LanguageMentor 英语私教") as language_mentor_app:
    with gr.Tab("对话练习"):
        ...
        conversation_chatbot = gr.Chatbot(
            placeholder="<strong>我是英语私教</strong><br><br>想和我聊什么话题都可以，记得用英语哦！",
            height=800,
        )
